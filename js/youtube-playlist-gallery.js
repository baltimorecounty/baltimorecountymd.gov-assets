...
baltimoreCounty.youtubePlaylistGallery = (function ($) {
  var documentationLink = "https://goo.gl/HbhJ1p",
    ...
    init = function (options) {
      ...
      var $youtubePlaylistGalleryTarget = options.target
          ? $(options.target.trim())
          : $(defaultOptions.target),
        templateSelector = options.templateSelector
          ? options.templateSelector
          : defaultOptions.templateSelector,
        playlistId = options.playlistId;

      if ($youtubePlaylistGalleryTarget.length === 0)
        throw (
          'The "target" option value must be supplied. Please see documentation at ' +
          documentationLink +
          "."
        );
      ...
      if (!playlistId || playlistId.length === 0)
        throw (
          'The "playlistId" option must be supplied. Please see documentation at ' +
          documentationLink +
          "."
        );
      ...
      getPlaylistDataFromYouTube(playlistId)
        .done(function (data) {
          ...
          if (playlistItems.length) {
            html = generateYouTubePlaylistHtmlWithHandlebars(
              playlistItems,
              $youtubePlaylistGalleryTarget,
              templateSelector
            );
          } else {
            html =
              '<div role="alert" class="alert-warning"><p>An error has occurred retreiving the videos from YouTube.</p></div>';
          }

          $youtubePlaylistGalleryTarget
            .html(html)
            .find(".youtube-playlist-item p a")
            .elliptical();
        ...
        .fail(function (data) {
          console.log(
            "Data load from YouTube failed. Response: " + JSON.stringify(data)
          );
        });
    },
    /**
    ...
    getPlaylistDataFromYouTube = function (playlistId) {
      var url =
        "//services.baltimorecountymd.gov/api/playlistgallery/" + playlistId;
        ...
        for (var i = 0; i < playlistData.length; i++) {
          if (playlistData[i].snippet) {
            try {
              ...
              console.log(
                "Incomplete data in the following snippet: " +
                  JSON.stringify(playlistData[i])
              );
            }
          }
        }
     ...
     */
    generateYouTubePlaylistHtmlWithHandlebars = function (
      youtubeItemInfoArr,
      $target,
      templateSelector
    ) {
      ...
      if (youtubeItemInfoArr.length > 6)
        html +=
          '<button type="button" class="contentButton dg_button">LOAD MORE</button>';
